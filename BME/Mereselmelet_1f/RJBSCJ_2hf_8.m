...
sigma_sys=1;             %A zajok szórásai állítható paraméterek
sigma_obs=1;
...
while((diff > epsilon) || (i < 3*N))
    idx = mod(i,N)+1;
    i = i + 1;
    y = c(:,idx).'* weight + noise_obs(idx);                %y(n)
    x = x + noise_sys + 1j*noise_sys;                       %x(n+1)
    y_b(i) = c(:,idx).'* weight_b(:,i);                     %y_b(n)
    y_delta = y - y_b(i);                                   %y_delta
    R = (((sqrt(R)*(i-1)) + y_delta)/(i)).^2;               %varianca becslő
    G(:,i) = P*c(:,idx)*inv(N*g(:,idx).'*P*c(:,idx) + R);   %G(n)
    weight_b(:,i+1) = weight_b(:,i) + G(:,i)*y_delta;       %x_b(n)
    P = (eye(N) - G(:,i)*c(:,idx).')*P + Q;                 %P(n+1)
    Ew = weight_b(:,i+1)-weight_b(:,i);                     %variancia becslő
    Q = (sqrt((sqrt(Q)*(i-1)).^2 + Ew*Ew.')/(i)).^2;
    if(i > 1)
        %leállási feltétel számítása
        change = weight_b(:,i) - weight_b(:,i-1);
        diff = norm(change)/norm(weight_b(:,i));
    end
end
figure(7)
plot(real(y_b))
